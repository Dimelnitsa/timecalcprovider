...
class ItemScreenWidget extends StatelessWidget {
  const ItemScreenWidget({
    Key? key,
  }) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final _itemEdited = context.watch<ItemScreenViewModel>().itemEdited;
    final _isVerse = context.select((ItemScreenViewModel value) => value.isVerse);
    final _isPicture = context.select((ItemScreenViewModel value) => value.isPicture);
    final _isTable = context.select((ItemScreenViewModel value) => value.isTable);
...
class StartTime extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final _itemEdited = context.read<ItemScreenViewModel>().itemEdited;
...
class DurationItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final _itemEdited =
    context.select((ItemScreenViewModel model) => model.itemEdited);
    return Text(
